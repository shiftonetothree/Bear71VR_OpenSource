...
const getOnClickRoot = require('../../util/getOnClickRoot');
...
const defaultParams = {
  colorNormal: new THREE.Color('#4D666E'),
  colorActive: new THREE.Color('#4D666E'),
...
function _onSelect () {
  if (this.state === States.Ground) {
    if (this.appState.gridInteractive && !this.map.isMoving && !this.rayEvents.isModalOpen && !this.appState.isChapterMenuOpen) {
      ...
      console.log("curPosition", this.position);
      this.map.moveCamera([this.position.x, this.position.z], undefined, undefined, undefined, true);
    }
  }else if(this.state === States.Actor){
    if(this.activeActor.onclick){
      this.activeActor.onclick();
    };
  }
};
...
Cursor.prototype.onEnterFrame = function(dt) {
  ...
  let hit = shouldRaycast ? this.raycaster.intersectObjects(this.colliders, true)[0] : null;
  ...
  if (collisionOwner && collisionOwner.collisionEnabled !== false) {
    ...
    }else if (newState === States.Actor){
      const object = getOnClickRoot(hit.object);
      __newStatePosition.copy(object.position);
      this.activeActor = object;
    }
