...
function update (dt) {
  ...
  if (!this.vr) {
    ...
    console.log(this.cameraControls.direction,this.cameraControls.position,this.cameraControls.up);
    __targetVec3.fromArray( this.cameraControls.direction );
    this.camera.position.y += this.getUserHeight();
    this.camera.lookAt(this.camera.parent.localToWorld(__targetVec3));
