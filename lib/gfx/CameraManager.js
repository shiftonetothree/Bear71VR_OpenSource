...
function update (dt) {
  ...
  if (!this.vr) {
    ...
    this.cameraFootstool.position.y = settings.userFootHeight || 0;
    ...
    const newDriection = (new THREE.Vector3()).fromArray(this.cameraControls.direction)
    if(!__targetVec3.equals(newDriection)){
      __targetVec3.copy( newDriection );
      this.cameraBody.updateWorldMatrix(false,true);
      this.camera.lookAt(this.camera.parent.localToWorld(__targetVec3.clone()));
    }
    // use under three@0.95.0
  ...
  location.style.left = this.cameraBody.position.x + "px";
  location.style.top = this.cameraBody.position.z + "px";
  
  const rotationYXZ = this.camera.rotation.clone();
  rotationYXZ.reorder("YXZ");
  location.style.transform = `rotateZ(${(-rotationYXZ.y/Math.PI)*180}deg)`;
}
