...
Map.prototype.moveAndRotateCamera = function(location, duration, id, onComplete = noop, height, white) {
  const newPosition = __tmpNewPosition.set(
    location.x,
  ...
  if(duration > 0) {
    gsap.to(this.cameraManager.cameraControls, duration, {
      id:`rotate${id}`,
      phi: this.cameraManager.cameraControls.phi,
      theta: rotate.y,
      ease: settings.cameraMovementEasingFunction,
      onUpdateScope: this,
      onComplete: onCompleteWrapper
...
Map.prototype.getCameraRotation = function (targetPosition) {
  ...
  if (this.cameraManager.cameraControls.theta - theta > Math.PI  ) {
    theta = theta + 3 / 2 * Math.PI;
  }
  if (this.cameraManager.cameraControls.theta - theta < -Math.PI  ) {
