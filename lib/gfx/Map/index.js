...
Map.prototype.moveAndRotateCamera = function(location, duration, onComplete = noop, height, white) {
  this.isMoving = true;
  const newPosition = __tmpNewPosition.set(
    location.x,
...
Map.prototype.getCameraRotation = function (targetPosition) {
  const rotateY = Math.atan2(this.cameraPosition.x - targetPosition.x, this.cameraPosition.z - targetPosition.z);
  const rotateX = Math.atan2(this.cameraPosition.y - targetPosition.y, this.cameraPosition.z - targetPosition.z);
  // Math.atan2(Math.sqrt(this.cameraPosition.x * this.cameraPosition.x + this.cameraPosition.z * this.cameraPosition.z), offset[1])
  return {
    x: rotateX,
    y: rotateY,
  };
...
Map.prototype.rotateCamera = function(location, duration) {
  ...
  const rotate = this.getCameraRotation(location);
  console.log("ggggggg", this.cameraManager.cameraControls);
  if(duration > 0) {
    gsap.to(this.cameraManager.cameraControls, duration, {
      phi: rotate.x,
      theat: rotate.y,
    ...
    this.camera.blindfold.blink(
      function() {
        //_this.cameraRotation.copy(newPosition);
        _this.updateMapForCamera();
      },
      onCompleteWrapper,
      white
