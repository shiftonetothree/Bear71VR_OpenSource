...
const defaultCarLikeG = { 
  [mapTileTypes.indexOf('road') * 8]: 'road', 
  [mapTileTypes.indexOf('railway') * 8]: 'railway'
}
...
function Map(params) {
  ...
  const mapTexture = assets.load(params.mapUrl, function(err, texture) {
    ...
    var allConnections = {};
    var roadlikeTileIndices = [];
    for(let key in params.carLikeG){
      roadlikeTileIndices.push(parseInt(key));
    }
  ...
  var rippleMap = new RippleMap({
    map: this,
    mapUrl: params.rippleMaterialUrl,
  ...
  this.cameraRotation = params.cameraRotation;
  this.cameraFootstoolPosition = params.cameraFootstoolPosition;
...
s.forEach(function(uniformName) {
  let param;
  Object.defineProperty(Map.prototype, uniformName, {
    set: function (val) {
      param = val;
      if(this.sections){
        this.sections.forEach(function(section) {
          section[uniformName] = val;
        });
      }
    },
    get: function(){
...
Map.prototype.moveCamera = function(location, duration, onComplete = noop, height, white) {
  ...
  if(duration === undefined || duration === null) {
    ...
    duration = distanceInMeters / settings.metersPerSecond;
  }
...
Map.prototype.moveAndRotateCamera = function(location, rotateLocation ,duration, id, onComplete = noop, height, white) {
  const newPosition = __tmpNewPosition.set(
    location.x,
    height !== undefined ? height : this.getHeightAtLocation(location.x, location.z),
    location.z
  );
  if(duration === undefined || duration === null) {
    const oldPosition = __tmpOldPosition.copy(this.cameraPosition);
  ...
  if (rotateLocation) {
    const rotate = this.getCameraRotation(rotateLocation);
      gsap.to(this.cameraManager.cameraControls, duration, {
        id:`rotate${id}`,
        phi: rotate.y,
        theta: rotate.x,
        ease: settings.cameraMovementEasingFunction,
...
Map.prototype.getCameraRotation = function (targetPosition) {
  const cameraWorldPosition = this.camera.parent.parent.getWorldPosition(new THREE.Vector3(0,0,0));
  let theta = Math.atan2(cameraWorldPosition.x - targetPosition.x, cameraWorldPosition.z - targetPosition.z);
  const x = Math.abs(cameraWorldPosition.z - targetPosition.z)
  const y = Math.abs(cameraWorldPosition.x - targetPosition.x)
  const z = Math.sqrt(x * x + y * y)
  const phi = Math.atan2(z, cameraWorldPosition.y + this.cameraFootstoolPosition.y  - targetPosition.y);
  if (Math.abs(this.cameraManager.cameraControls.theta - theta > Math.PI)) {
    // theta = THREE.Math.degToRad( 360 + THREE.Math.radToDeg(theta));
    if(theta < 0) {
   ...
   /** 第二种写法 */
  // const distanceBetweenCameraAndPoi = world.distanceTo(targetPosition);
  // const phi = Math.acos((world.y - targetPosition.y) / distanceBetweenCameraAndPoi);
...
Map.prototype.rotateCamera = function(location, duration) {
  this.isMoving = true;
  gsap.killTweensOf(this.cameraManager.cameraControls);
  var _this = this;
  function onCompleteWrapper() {
    _this.isMoving = false;
  }
  const rotate = this.getCameraRotation(location);
  if(duration > 0) {
    gsap.to(this.cameraManager.cameraControls, duration, {
      id: "rotateCamera",
      phi: rotate.y,
...
Map.prototype.setAngle = function (phi, theta, distance) {
  const controls= this.cameraManager.cameraControls;
  var r = distance || this.camera.position.distanceTo(controls.target);
...
Map.prototype.cameraChangeHeight = function (height, duration){
  const newPosition = __tmpNewPosition.set(
    this.cameraFootstoolPosition.x,
    height,
    this.cameraFootstoolPosition.z
  );
  gsap.killTweensOf(this.cameraFootstoolPosition);
  var _this = this;
  function onCompleteWrapper() {
    _this.isMoving = false;
  }
  if(duration > 0) {
    gsap.to(this.cameraFootstoolPosition, duration, {
      id: "changeHeight",
      x: newPosition.x,
      y: newPosition.y,
      z: newPosition.z,
    ...
    this.camera.blindfold.blink(
      function() {
        _this.cameraFootstoolPosition.copy(newPosition);
        _this.updateMapForCamera();
      },
      onCompleteWrapper,
      white
...
Map.prototype.stopMovingCamera = function() {
  const rotations = gsap.getTweensOf(this.cameraManager.cameraControls);
  const  move = gsap.getTweensOf(this.cameraPosition);
  this.stopMove = true;
  rotations.map(item => {
    if (item.vars.id !== "rotateCamera") {
      item.kill();
    }
    
  });
  move.map(item => {
    if (item.vars.id !== "moveCamera") {
      item.kill();
    }
  });
}
...
Map.prototype.changeSpritesheet = function(url) {
  const spritesheetTexture = assets.load(url,undefined,()=>{
    this.spritesheetTexture = spritesheetTexture;
  });
  spritesheetTexture.premultiplyAlpha = true;
  
}
Map.prototype.updateSectionOffset = function(offset) {
  this.sections.forEach(function(section) {
    section.updateGroundPlaneOffset(offset);
  });
}
Map.prototype.moveWithCurve = function(targets, durtation, pointsNumber, showLine) {
  let line = addLines(targets).lineMesh;
  let curve= addLines(targets).curve;    //绘制贝塞尔曲线
  if(showLine){
    this.scene.add(line);
  }
  this.stopMove = false;
//取curve的50个点
  ...
  function move () {
    if(!that.stopMove) {
      that.moveAndRotateCamera(points[index], points[index + 1], durtation, index);
      index++;
  ...
  let a = setInterval(move(), durtation * 1000);
}
// 添加线条
