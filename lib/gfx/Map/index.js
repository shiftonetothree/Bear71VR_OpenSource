...
function Map(params) {
  ...
  const mapTexture = assets.load(params.mapUrl, (err,texture)=> this.ready(true,false,texture));
  ...
  getMapSections(sections,this,rippleMap,scene,params,spritesPerMeter,pointsPerSegment,segmentSpacing,segmentSize,halfSize).then(
    ()=>{
      this.ready(false, true);
    }
  );
...
Map.prototype.ready = function(mapTextureReady,mapSectionsReady,texture){
  if(mapTextureReady && texture){
    this.mapTextureReady = true;
    this.texture = texture;
  }

  if(mapSectionsReady){
    this.mapSectionsReady = true;
    this.initSpriteScale();
  }

  if(this.mapTextureReady && this.mapSectionsReady){
    this.initMapTexture(this.texture);
    this.camera.blindfold.animateOut();
    this.isReady = true;
    this.onReadyCallbacks.forEach(function(cb){
      cb();
    });
    // this.moveCamera(
    ...
    //   0
    // );
    this.emitter.emit(this.LOADED);
    
  }
}

Map.prototype.initMapTexture = function(texture) {
  const _this = this;
  _this.mapTexture.magFilter = THREE.NearestFilter;
  _this.mapTexture.minFilter = THREE.NearestFilter;
  _this.mapTexture.flipY = false;
  _this.mapTexture.generateMipmaps = false;
  _this.mapTexture.needsUpdate = true;
  ...
  _this.sections.forEach(function(section) {
    section.spriteSpacing = image.width / _this.spritesPerMeter;
  });
  ...
  var elevationScale = _this.elevationScale;
  var now = Date.now();
  ...
  this.roadlikeConnections = allConnections;
...
Map.prototype.initSpriteScale = function() {
...
async function getMapSections (sections,map,rippleMap,scene,params,spritesPerMeter,pointsPerSegment,segmentSpacing, segmentSize,halfSize){
  var sectionParams = {
    cameraPosition: params.cameraPosition,
    map: map,
    groundPlaneOffset: params.groundPlaneOffset,
  ...
  for (var iy = 0; iy < params.segments; iy++) {
    for (var ix = 0; ix < params.segments; ix++) {
      ...
      var section = await getMapSection(sectionParams);
      scene.add(section);
      var s = segmentSize;
      console.log(s);
      ...
      // console.log(section.spriteScale);
      sections.push(section);
      
      console.log(`inited mapSection x:${ix} y:${iy}`);
    }
  }
}


async function getMapSection (sectionParams){
