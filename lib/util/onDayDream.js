...
function clear() {
	const __callbacksActiveCopy = __callbacksActive.concat();
	for(let callback of __callbacksActiveCopy){
		__removeIfHas(__callbacksActive, callback);
	}
	const __callbacksSleepingCopy = __callbacksSleeping.concat();
	for(let callback of __callbacksSleepingCopy){
		__removeIfHas(__callbacksSleeping, callback);
	}
}

onDayDream.remove = remove;
onDayDream.clear = clear;
