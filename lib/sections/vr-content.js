...
const getTracks = require('../gfx/track');
...
function setVrContent ({ app, state, mapConfig, onLoaded }) {
  ...
  map = new Map(
    ...
    cameraRotation: app.cameraBody.rotation,
    cameraFootstoolPosition: app.cameraFootstool.position,
    cameraBody: app.cameraBody,
  ...
  mapLoaded = function(){
    ...
    mapCollider = new MapCollider({
      map,
      cameraPosition: app.cameraBody.position,
      cameraRotation: app.cameraBody.ratation
    });
    ...
    getLocations(mapConfig.locations,
      (locations)=>{
        for(let location of locations){
          ...
          location.dispose = function(){
            map.rippleMap.unregisterRippler(location);
            this.onEnterFrame = null;
            this.onclick = null;
            this.dispose = null;
          };
          location.position.y = location.position.y + map.getHeightAtLocationFast(location.position.x,location.position.z);
          if(location.onclick){
        ...
        getTracks(mapConfig.carLikes, map.roadlikeConnections, (spiretes)=>{
          for(let spirete of spiretes){
            spirete.onEnterFrameOld = spirete.onEnterFrame;
            spirete.onEnterFrame = function(dt){
              if(spirete.onEnterFrameOld){
                spirete.onEnterFrameOld(dt);
              }
              const target = app.cameraBody.position.clone();
              target.y = this.position.y;
              this.lookAt(target);
            ...
            if(spirete.onclick){
              cursor.addCollider(spirete);
            }
            spirete.dispose = function(){
              map.rippleMap.unregisterRippler(spirete);
              spirete.onEnterFrame = null;
              spirete.onEnterFrameOld = null;
              spirete.onclick = null;
              this.dispose = null;
            };
            scene.add(spirete);
            map.rippleMap.registerRippler(spirete);
          }
          // not needed in daydream
          // findNorth();
          if(onLoaded){
            // see setTimeout in PlaneCollider
            window.setTimeout(()=>onLoaded(destroy, map),1500);
          }
        });
