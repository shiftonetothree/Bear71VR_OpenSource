...
function GridWorldApp (params = {},canvas) {
  ...
  if(params.showAxesHelper){
    scene.add( new THREE.AxesHelper( 200 ) );
    scene.add( new THREE.CameraHelper(cameraManager.camera));
    this.cameraBodyBox = new THREE.BoxHelper(cameraManager.cameraBody);
    scene.add( this.cameraBodyBox);
    this.cameraFootstoolBox = new THREE.BoxHelper(cameraManager.cameraFootstool)
    scene.add( this.cameraFootstoolBox);
  }
...
function _onEnterFrame () {
  ...
  this.cameraBodyBox && this.cameraBodyBox.update();
  this.cameraFootstoolBox && this.cameraFootstoolBox.update();
...
function registerOnResize(callback) {
  ...
  callback(this.width, this.height, false);
}
...
function _resize (w, h) {
  ...
  for (var i = 0; i < this.onResizeCallbacks.length; i++) {
    this.onResizeCallbacks[i](width, height, false);
  }
