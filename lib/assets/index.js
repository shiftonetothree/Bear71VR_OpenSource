...
const isBase64 = (path) => path !== null && path.indexOf("data:image/png;base64") === 0;
...
assets.load = load;
assets.loadPromise = loadPromise;
...
function preload(url, cb) {
  ...
  if (isImage(path.extname(url)) || isBase64(url)) {
    loadImage(url, cb);
  } else {
    load(url, cb);
  }
...
async function loadPromise(url, opt) {
